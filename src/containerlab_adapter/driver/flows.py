...
Cumulus has no equivalent first-class concept — flows on a running
fabric emerge from packet captures, sFlow, or application-level
reporting.
...
def get_flow_records(client: ContainerlabClient) -> dict[str, Any]:
    ...
    if Cumulus VX exposes it on the container interfaces,
    (b) tcpdump-based capture on hosts with offline FCT analysis,
    (c) application-level reporting from the traffic generator
    (iperf JSON output is workable).
    """
